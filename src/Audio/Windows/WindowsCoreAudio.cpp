...
int WMMDevice::BufferData(char * data, int maxData)
{
	...
	if (!playing)
	{
		...
		int bytesBuffered = min(maxData, bufferFrameCount);
		// Load the initial data into the shared buffer.
		// Add some sine-data
		float * fBuf = (float*) buffer;
		for (int i = 0; i < bytesBuffered / 4; ++i)
		{
			fBuf[i] = sin(i * 0.01f);
		}

		// https://msdn.microsoft.com/en-us/library/windows/desktop/dd368244%28v=vs.85%29.aspx
		hr = renderClient->ReleaseBuffer(bytesBuffered, 0);
		assert(hr == S_OK);
		CANCEL_ON_ERROR;

		// Calculate the actual duration of the allocated buffer.
		hnsActualDuration = (double)REFTIMES_PER_SEC *
							bytesBuffered / pwfx->nSamplesPerSec;
...
*/
	...
	int bytesToBuffer = min(bytesAvailable, maxData);
	...
	int numFramesWritten = numFramesAvailable;
