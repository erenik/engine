...
/// Loads into texture.
bool SavePNG(String toFile, Texture * intoTexture)
{
	Timer timer;
	timer.Start();
	DataStream stream, outputStream;
...
#define dataPtr (*dataPtrPtr)
	...
	uchar png[4] = {0x89, 0x50, 0x4E, 0x47};
	stream.PushBytes(png, 4);	
	// push 8 bytes of info..?
	stream.PushBytes(png, 8);
...
//	ret = deflateInit2(&strm, 15, method, windowBits, memLevel, strategy);
//	if (ret != Z_OK)
//		RETURN_ERROR();
//	
//	bool streamEndedCorrectly = false;
