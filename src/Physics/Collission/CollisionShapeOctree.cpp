...
int CollisionShapeOctree::FindCollissions(Entity * targetEntity, List<Collission> & collissionList, int entrySubdivisionLevel){
...
*/
int CollisionShapeOctree::FindCollissions(Entity * targetEntity, List<Collission> & collissionList, Matrix4f & localTransform, int entrySubdivisionLevel/* = -1*/)
{
	...
	for (int i = 0; i < triangles.Size(); ++i){
		...
		if (targetEntity->physics->type == ShapeType::MESH){
			...
			assert(targetEntity->physics->type != ShapeType::MESH && "CollisionShapeOctree::FindCollissions mesh-triangle collission where unwanted!");
		...
		switch(targetEntity->physics->physicsShape){
			...
			default:
				std::cout<<"\nUnsupported shape-type for specified mesh when checking collissions vs. Triangles in CollisionShapeOctree::FindCollissions!";
				assert(false && "Unsupported shape-type for specified mesh when checking collissions vs. Triangles in CollisionShapeOctree::FindCollissions!");
				break;
		}
		++collissionsTested;
	}
	...
	if (this->subdivision == entrySubdivisionLevel){
		...
		for (int i = 0; i < childList.Size(); ++i){
			...
			if (ch->IsEntityInside(targetEntity, localTransform) == OUTSIDE)
				continue;
			collissionsTested += ch->FindCollissions(targetEntity, collissionList, localTransform, entrySubdivisionLevel);
		}
	...
	/// Process children if subdivision level is higher (further down the tree.)
		...
		for (int i = 0; i < childList.Size(); ++i){
			...
			if (child->IsEntityInside(targetEntity, localTransform) == OUTSIDE)
				continue;
			collissionsTested += child->FindCollissions(targetEntity, collissionList, localTransform, entrySubdivisionLevel);
		}
		return collissionsTested;
	}
	...
	assert(false && "CollisionShapeOctree::FindCollissions at a point where it shouldn't be!");
	return 0;
}
