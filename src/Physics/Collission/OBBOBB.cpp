...
bool OBBOBBCollission(OrientedBoundingBox * obb1, OrientedBoundingBox * obb2, Collission & data){
    ...
    deepestCollission.distanceIntoEachOther = -1000000.0f;
    Collission tempData;
...
#define AXES_TO_TEST	15
	...
	if (intersectionAxis.MaxPart() > 0){
		data.preliminaryCollissionNormal = intersectionAxis;
		data.results |= PRELIMINARY_COLLISSION_NORMAL;
//		std::cout<<"\nEstimated collission-normal: "<<data.collissionNormal;
		tempData.results |= PRELIMINARY_COLLISSION_NORMAL;
		tempData.preliminaryCollissionNormal = data.preliminaryCollissionNormal;
	}
...
*/
	data.collissionNormal = data.preliminaryCollissionNormal;
...
//		std::cout<<"\nEdge-Edge Collission!";
	...
	// Edge-Face intersectionnnnnnn
		...
		for (int i = 0; i < OrientedBoundingBox::FACES; ++i){
			...
			if (f->normal.DotProduct(data.collissionNormal) >= 0.9f)
				targetFace = f;
			f = obb2->faceList[i];
			if (f->normal.DotProduct(data.collissionNormal) >= 0.9f)
				targetFace = f;
	...
	//	std::cout<<"\n OMG! Face-Face Collisshuuuuiiion! DAYOU";
		...
		for (int i = 0; i < OrientedBoundingBox::FACES; ++i){
			...
			if (f->normal.DotProduct(data.collissionNormal) >= 0.9f)
				targetFace1 = f;
			f = obb2->faceList[i];
			float dotProduct = f->normal.DotProduct(data.collissionNormal);
			if (dotProduct <= -0.9f)
				targetFace2 = f;
		}
	...
	/// Seprating Axises tested! Collission is probable.
    ...
    for (int i = 0; i < 8; ++i){
        ...
        if (collided){
            ...
            std::cout<<"\nDistance into each other: "<<tempData.distanceIntoEachOther<<" deepest: "<<deepestCollission.distanceIntoEachOther;
            if (tempData.distanceIntoEachOther > deepestCollission.distanceIntoEachOther){
                deepestCollission = tempData;
    ...
    if (collissionImminent){
        ...
        if (data.collissionPoint.MaxPart() > 0.0f &&
            data.collissionNormal.MaxPart() > 0.0f)
            return true;
        else {
            std::cout<<"\nCollission point or normal is wacked out yo! Cannot be considered a serious collission! o.o";
            std::cout<<"\nPoint: "<<data.collissionPoint;
            std::cout<<"\nNormal: "<<data.collissionNormal;
        }
    }
	...
	for (int i = 0; i < obb1->edgeList.Size(); ++i){
		...
		for (int j = 0; j < obb2->faceList.Size(); ++j){
			...
			bool collided = EdgeFaceCollission(edge, face, tempData);
