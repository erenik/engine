	...
	for (int i = 0; i < face->triangleList.Size(); ++i){
		...
		//	data.distanceIntoEachOther = t;
			...
			if (data.results & PRELIMINARY_COLLISSION_NORMAL){
				...
				float dot = data.preliminaryCollissionNormal.DotProduct(tmpNormal);
				if (dot < 0.5f){
					std::cout<<"\nWARNING: Collission normal and preliminary collission-normal do not coincide!";
				}
				...
				if (edge->faceList.Size() < 2)
					data.collissionNormal = Vector3f(0,1,0);
				else 
					data.collissionNormal = (edge->faceList[0]->normal + edge->faceList[1]->normal).Normalize();
			}
			return true;
		}
	}
