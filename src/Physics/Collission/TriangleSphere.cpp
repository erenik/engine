...
bool TriangleSphereCollission(Entity * triEntity, Entity * sphereEntity, Collission &data){
...
bool TriangleSphereCollission(Triangle * triangle, Sphere * sphere, Collission &data){
	...
	for (int i = 0; i < PLANE_PLANES; ++i){
		...
		if (planeFrustum[i].Distance(sphere->position) > 0){ // sphereEntity->physics->physicalRadius)
			colliding = false;
			break;
		}
	}
	...
	if (colliding){
		data.collissionNormal = triangle->normal;
	...
	//		std::cout<<"\nDistance to sphere,: "<< distanceToSphere ;
		...
		data.collissionNormal = projectedPointToSphere.NormalizedCopy();
