...
const float ScrollerPlayerState::DEFAULT_MOVEMENT_SPEED = 0.25f;

ScrollerPlayerState::ScrollerPlayerState(Entity * entity)
...
void ScrollerPlayerState::OnEnter(){
}

/// Main processing function
void ScrollerPlayerState::Process(float timePassed){
	// 
...
void ScrollerPlayerState::OnExit(){
}

void ScrollerPlayerState::ProcessMessage(Message * message){
	std::cout<<"\nScrollerPlayerState::ProcessMessage: ";
	switch(message->type){
		case MessageType::STRING: {
			...
			if (s == "walkLeft")
				...
				if (!tmap){
					std::cout<<"\nERROR: ScrollerPlayerState::ProcessMessage: No Active map to interact on!";
					return;
				}
...
/// Wosh.
void ScrollerPlayerState::DisableMovement(){
	movementEnabled = false;
}
void ScrollerPlayerState::EnableMovement(){
	movementEnabled = true;
}

/// One step in the looking-direction.
Vector2i ScrollerPlayerState::GetInteractionTile(){		
...
void ScrollerPlayerState::UpdateQueuedMovement(){
	if (up)
...
void ScrollerPlayerState::HandleMovement(float & timePassed){
...
/*
		...
		if (map->IsTileVacant(reqTileX, reqTileY)){
			...
			if (success){
				...
				std::cout<<"\nScrollerPlayerState::HandleMovement Moved to: "<<newPosition;
