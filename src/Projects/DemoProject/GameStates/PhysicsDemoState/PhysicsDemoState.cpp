...
Selection PhysicsDemoState::GetActiveSelection(){
    return editorSelection;
};

void PhysicsDemoState::CreateUserInterface(){
	if (ui)
		delete ui;
	this->ui = new UserInterface();
	ui->Load("gui/DemoProject/PhysicsTest.gui");
}

void PhysicsDemoState::OnEnter(GameState * previousState){
...
void PhysicsDemoState::OnExit(GameState *nextState){
	// Begin loading textures here for the UI
...
void PhysicsDemoState::ProcessMessage(Message * message){
	std::cout<<"\nPhysicsDemoState::ProcessMessage: ";
	switch(message->type){
...
void PhysicsDemoState::OnSelectionUpdated(){
	LOG("Entities selected: "+String::ToString(editorSelection.Size()));

//	std::cout<<"\nPhysicsDemoState::OnSelectionUpdated()";
...
void PhysicsDemoState::MouseClick(bool down, int x, int y, UIElement * elementClicked){
...
void PhysicsDemoState::MouseRightClick(bool down, int x, int y, UIElement * elementClicked){
    if (down != rButtonDown){
        /// Mouse press
        if (down){
            startMouseX = (float)x;
...
void PhysicsDemoState::MouseMove(float x, float y, bool lDown, bool rDown, UIElement * elementOver){
...
void PhysicsDemoState::MouseWheel(float delta){
	Camera * camera = Graphics.cameraToTrack;
...
void PhysicsDemoState::LoadMap(String fromFile){
	editorSelection.Clear();
...
void PhysicsDemoState::TranslateActiveEntities(Vector3f distance){
	Physics.QueueMessage(new PMSetEntity(TRANSLATE, editorSelection, distance));
}
void PhysicsDemoState::SetScaleActiveEntities(Vector3f scale){
	Physics.QueueMessage(new PMSetEntity(SET_SCALE, editorSelection, scale));
}
void PhysicsDemoState::ScaleActiveEntities(Vector3f scale){
	Physics.QueueMessage(new PMSetEntity(SCALE, editorSelection, scale));
}
void PhysicsDemoState::RotateActiveEntities(Vector3f rotation){
...
void PhysicsDemoState::HandleDADFiles(List<String> & files){
...
void PhysicsDemoState::ReturnToLastActiveMap(){
...
void PhysicsDemoState::SetMode(int mode){
...
#define MAKE_VISIBLE(b) Graphics.QueueMessage(new GMSetUIb(b, GMUI::VISIBILITY, true));
	switch(mode){
		...
		case NAVMESH:
			...
			break;
		default:
			std::cout<<"\nERROR: Bad mode supplied in PhysicsDemoState::SetMode, returning...";
			return;
	}
	editMode = mode;
};
